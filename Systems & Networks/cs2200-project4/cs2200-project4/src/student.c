...
{
    // returns functional priority
    return ((double) (process->priority) + ((double)current_time - (double) (process->enqueue_time)) * ((double) age_weight));
}
...
void enqueue(queue_t *queue, pcb_t *process)
{
    ...
    if (queue->tail) { // if tail is not null...

        //if (scheduler_algorithm == PA) {                        // FIX PLSS DONT NEED THIS
            // if (queue->head->priority < process->priority) {
            //     process->next = queue->head;
            //     queue->head = process;
            //     process->enqueue_time = get_current_time();
            // }
            // else { // if process priorirty is less than head priority
            //     pcb_t *curr = queue->head;

            //     while (curr->next && (curr->next->priority) < process->priority) {  //logic may be wrong.
            //         curr = curr->next;
            //     }
...
pcb_t *dequeue(queue_t *queue)
{
    ...
    unsigned int currT = get_current_time();
    if (is_empty(queue)) {
        return 0;
    }
    ...
    if (scheduler_algorithm == PA) {                                    // PA specific code. everything else works 
        pcb_t *curr = output;
        pcb_t *prev = 0;
        pcb_t *high = queue->head;
        double highest_priority = priority_with_age(currT, high);

        while (curr->next) {
            double curr_priority = priority_with_age(currT, curr->next);
        ...
        output = high;
        output->priority = highest_priority;
        if (prev) {
        ...
        if (output == queue->tail) {
            queue->tail = prev;
        }
    } else {                                                            //end of PA specific code. pls don't break anything
        queue->head = queue->head->next;
...
extern void wake_up(pcb_t *process)
{
    ...
    if (scheduler_algorithm == PA) {                            //PA speciifc code part 2
        unsigned int preempt = 0;
        unsigned int min = 0;
        pthread_mutex_lock(&current_mutex);                     //mutex start

        for (unsigned int i = 0; i < cpu_count; i++) {
            if (!current[i]) {
                pthread_mutex_unlock(&current_mutex);           //mutex end 1
                return;
            } else {
                if (current[i]->priority < priority_with_age(current_time, process)) {
                    preempt = 1;
                }
        ...
        pthread_mutex_unlock(&current_mutex);                   //mutex end frfr
        if (preempt) {
            force_preempt(min);
        }
    }                                                           // END OF PA specific code
}
