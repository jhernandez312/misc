...
void system_init(void) {
    ...
    // there are 2^6 entries in the frame table (6 bits pfn)
    frame_table = (fte_t*) mem;
    memset(mem, 0, PAGE_SIZE); //frame_table is the same size as a page
...
uint8_t mem_access(vaddr_t addr, char access, uint8_t data) {
    ...
    vpn_t vPageNum = vaddr_vpn(addr); //getting virtual page number
    uint16_t offset = vaddr_offset(addr); //offset
    stats.accesses++;

    pte_t *entry = (pte_t*)(mem + PTBR*PAGE_SIZE + vPageNum);

    if (!entry->valid) {
        page_fault(addr);
    }

    pfn_t pPageNum = entry->pfn;
    fte_t *physEntry = frame_table + pPageNum;
    //paddr_t paddr = (paddr_t)(mem + pPageNum*PAGE_SIZE + offset); //concatenating PFN + offset
    paddr_t paddr = (paddr_t)(pPageNum << OFFSET_LEN) | offset;
    /* Either read or write the data to the physical address
       depending on 'rw' */
    physEntry->referenced = 1;
