...
void undrawImageDMA(int row, int col, int width, int height, const u16 *image) {
  for(int i = 0; i < height; i++) {
      ...
      DMA[DMA_CHANNEL_3].cnt = DMA_ON | width | DMA_SOURCE_INCREMENT | DMA_DESTINATION_INCREMENT;
  }
}
...
void fillScreenDMA(volatile u16 color) {
    DMA[3].src = &color;
    DMA[3].dst = videoBuffer;
    DMA[3].cnt = 240 * 160 | DMA_ON | DMA_16 | DMA_AT_VBLANK | DMA_SOURCE_FIXED | DMA_DESTINATION_INCREMENT;
}
